      ...
      <section className="py-12 bg-background">
        <div className="container mx-auto px-4">
          {loading ? (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="px-2 py-1 rounded-full bg-muted text-muted-foreground text-xs">
                        {difficultyMap[test.difficulty] || test.difficulty}
                      </span>
                    </div>
                    <h3 className="text-lg font-semibold mb-4">
                      {lang === 'en' ? test.title_en : lang === 'ru' ? test.title_ru : test.title_uz}
                    </h3>
                    <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6">
                      <div className="flex items-center gap-1">
                        <HelpCircle className="w-4 h-4" />
                        <span>{test.questions_count} {lang === 'en' ? 'questions' : lang === 'ru' ? 'вопросов' : 'savol'}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Clock className="w-4 h-4" />
                        <span>{test.duration} {lang === 'en' ? 'min' : lang === 'ru' ? 'мин' : 'min'}</span>
                      </div>
                    </div>
                    <Button className="w-full bg-gradient-primary hover:opacity-90">
                      {lang === 'en' ? 'Start Test' : lang === 'ru' ? 'Начать тест' : 'Testni boshlash'}
                    </Button>
                  </div>
                );
              })}
            ...
            <div className="text-center text-muted-foreground">Testlar mavjud emas</div>
          )}
        </div>
      </section>
    </Layout>
